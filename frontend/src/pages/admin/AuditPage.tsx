  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-wrap gap-2">
        {[['access', 'Logs de acceso'], ['tickets', 'Logs de tickets'], ['user-devices', 'Dispositivos / IP']] .map(([value, label]) => (
      ...
      <section className="panel overflow-x-auto p-0">
        {!records.length ? <div className="p-5"><Empty text="No hay registros en esta bandeja." /></div> : (
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-left text-slate-500"><tr><th className="px-4 py-4">Usuario</th><th className="px-4 py-4">Acción</th><th className="px-4 py-4">Ticket / Mensaje</th><th className="px-4 py-4">IP</th><th className="px-4 py-4">User-Agent</th><th className="px-4 py-4">Fecha</th></tr></thead>
            <tbody>{records.map((record) => (
              <tr key={record.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-3 font-medium">{record.action.replaceAll('_', ' ')}</td>
