    ...
    <div className="flex min-h-screen items-center justify-center bg-cecasem-mist p-4">
      <div className="w-full max-w-md panel p-8">
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <Field label="Contraseña actual" required><PasswordInput value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></Field>
          <Field label="Nueva contraseña" required><PasswordInput value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" required /></Field>
          <Field label="Confirmar nueva contraseña" required><PasswordInput value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" required /></Field>
